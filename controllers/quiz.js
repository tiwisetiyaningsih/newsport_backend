...
// CREATE: untuk menambahkan data kedalam quiz
exports.create = async (req, res) => {
    try {
        const data = await Quiz.create(req.body)
        res.json({
            message: 'asik data berhasil di tambahkan',
            data: data,
        })
    } catch (error) {
        res.status(500).json({
...
exports.update = async (req, res) => {
    ...
    try {
        ...
        res.json({
            message: 'Quizzes with updated successfully.',
            data: quiz,
        })
    } catch (error) {
        res.status(500).json({
...
// Menampilkan atau mengambil data quiz berdasarkan category tertentu
...
// Menampilkan atau mengambil data quiz berdasarkan level tertentu
